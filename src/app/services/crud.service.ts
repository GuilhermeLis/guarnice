...
export class CRUDService {
  ...
  readSelection(path: string, orderBy: string, ascOrDesc: any, where: any, operation: any, valor: any){
    return this.FSDB.collection(path, ref =>
      ref.where(where,operation,valor).orderBy(orderBy, ascOrDesc))
      .snapshotChanges().pipe(
        map(actions => actions.map(a => {
          const data = a.payload.doc.data();
          const id = a.payload.doc.id;
  ...
  readOnePromise(path: string,doc : string,orderBy?: string, ascOrDesc?:any){
    return this.FSDB.collection(path).doc(doc).get().toPromise();
  }
